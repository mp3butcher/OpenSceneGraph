...
void VertexArrayState::assignTexCoordArrayDispatcher(unsigned int numUnits)
{
    ...
    for(unsigned int i=0; i<_texCoordArrays.size(); ++i)
    {
        ...
        if (!_state->getUseVertexAttributeAliasing())
        {
            _texCoordArrays[i] = new TexCoordArrayDispatch(i, _basevertex);
        }
        else
#endif
        {
            ...
            _texCoordArrays[i] = new VertexAttribArrayDispatch(_state->getTexCoordAliasList()[i]._location, _basevertex);
        }
    }
}
...
void VertexArrayState::assignVertexAttribArrayDispatcher(unsigned int numUnits)
{
    ...
    for(unsigned int i=0; i<_vertexAttribArrays.size(); ++i)
    {
        ...
        _vertexAttribArrays[i] = new VertexAttribArrayDispatch(i,_basevertex);
    }
}
...
void VertexArrayState::setInterleavedArrays( osg::State& , GLenum format, GLsizei stride, const GLvoid* pointer)
{
