...
struct FindPointSharing
{
    ...
    FindPointSharing(){_result[0]=0xFFFFFFFF;_result[1]=0xFFFFFFFF;_result[2]=0xFFFFFFFF;}

    void setTriangle(unsigned int p1, unsigned int p2, unsigned int p3) { _p[0]=p1;_p[1]=p2;_p[2]=p3;
                                                                         _result[0]=p1;_result[1]=p2;_result[2]=p3;
                                                                        }
        ...
        switch(mode)
        {
            ...
            newmode=osg::PrimitiveSet::TRIANGLES_ADJACENCY;
                ...
                for(IndexPointer  iptr=indices;iptr<ilast;iptr+=3){
                    ...
                    FindIndexSharing2Point finder;finder.setTriangle
            ...
            newmode=osg::PrimitiveSet::TRIANGLES_ADJACENCY;
                ...
                {
                    if ((i%2)) {FindIndexSharing2Point finder;finder.setTriangle(*(iptr)+ basevertex,*(iptr+2)+ basevertex,*(iptr+1)+ basevertex);}
                    else       {FindIndexSharing2Point finder;finder.setTriangle(*(iptr)+ basevertex,*(iptr+1)+ basevertex,*(iptr+2)+ basevertex);}
                }
                break;
            }
            case(GL_QUADS):
            ...
            newmode=osg::PrimitiveSet::TRIANGLES_ADJACENCY;
                ...
                {
                    FindIndexSharing2Point finder;finder.setTriangle(*(iptr)+ basevertex,*(iptr+1)+ basevertex,*(iptr+2)+ basevertex);
                    FindIndexSharing2Point finder2;finder2.setTriangle(*(iptr)+ basevertex,*(iptr+2)+ basevertex,*(iptr+3)+ basevertex);
                }
                break;
            }
            case(GL_QUAD_STRIP):
            ...
            newmode=osg::PrimitiveSet::TRIANGLES_ADJACENCY;
                ...
                {
                    FindIndexSharing2Point finder;finder.setTriangle(*(iptr)+ basevertex,*(iptr+1)+ basevertex,*(iptr+2)+ basevertex);
                    FindIndexSharing2Point finder2;finder2.setTriangle(*(iptr+1)+ basevertex,*(iptr+3)+ basevertex,*(iptr+2)+ basevertex);
                }
                break;
            }
            case(GL_POLYGON): // treat polygons as GL_TRIANGLE_FAN
            case(GL_TRIANGLE_FAN):
            {
                ...
                for(GLsizei i=2;i<count;++i,++iptr)
                {
                    FindIndexSharing2Point finder;finder.setTriangle(first+ basevertex,*(iptr)+ basevertex,*(iptr+1)+ basevertex);
                }
                break;
            }
            case(GL_POINTS):
...
void MakeDrawElementAdjacency::generate_adjacency(osg::Geometry & geometry,unsigned int notfoundindex) const
{
    ...
    osg::Geometry::DrawElementsList drelist;    geometry.getDrawElementsList(drelist);
...
GenerateAdjacency  genadj(de);
de.accept(genadj);
///copy back
//DEBUGde.setMode(genadj.newmode);
de.clear();
