...
public:
    ...
    virtual WriteResult writeObject(const osg::Object& obj,std::ostream& fout,const osgDB::ReaderWriter::Options* local_opt=NULL) const {
        std::string fullFileName = local_opt->getPluginStringData("STREAM_FILENAME");
        return writeFile(OBJECT, &obj, fout, local_opt);
...
osgDB::ReaderWriter::ReadResult ReaderWriterGZ::readFile(ObjectType objectType, osgDB::ReaderWriter* rw, std::istream& fin, const osgDB::ReaderWriter::Options *options) const
{
    ...
    {
    case(OBJECT):
        return rw->readObject(fin,options);
    case(ARCHIVE):
        return rw->openArchive(fin,options);
    case(IMAGE):
        return rw->readImage(fin,options);
    case(HEIGHTFIELD):
        return rw->readHeightField(fin,options);
    case(NODE):
...
osgDB::ReaderWriter::WriteResult ReaderWriterGZ::writeFile(ObjectType objectType, const osg::Object* object, osgDB::ReaderWriter* rw, std::ostream& fout, const osgDB::ReaderWriter::Options *options) const
{
    ...
    {
    case(OBJECT):
        return rw->writeObject(*object, fout, options);
    case(IMAGE):
        return rw->writeImage(*(dynamic_cast<const osg::Image*>(object)), fout, options);
    case(HEIGHTFIELD):
        return rw->writeHeightField(*(dynamic_cast<const osg::HeightField*>(object)), fout, options);
    case(NODE):
        return rw->writeNode(*(dynamic_cast<const osg::Node*>(object)), fout,options);
    default:
        break;
    }
...
osgDB::ReaderWriter::WriteResult ReaderWriterGZ::writeFile(ObjectType objectType, const osg::Object* object,   std::ostream& fout, const osgDB::ReaderWriter::Options *options) const
{
    std::string fullFileName = options->getPluginStringData("STREAM_FILENAME");
    std::string ext = osgDB::getLowerCaseFileExtension(fullFileName);
    if(ext == "gz"){
        fullFileName=osgDB::getNameLessExtension(fullFileName);
        ext = osgDB::getLowerCaseFileExtension(fullFileName);
    }
